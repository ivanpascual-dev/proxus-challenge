...
| **modo práctica**        | Sin reloj ni penalización, hay pistas, el material a la vista y el chat del tutor; corrige al entregar | Campo `mode` del **intento**             |
...
- **Resolver**: `AssessmentSolver`, nuevo, sustituye a `ExerciseSolver`. Lee de `/solvable`. En los dos
  modos la corrección sale al entregar. Práctica no tiene reloj ni penalización, y sí pistas, el material
  a la vista y el chat del tutor abierto.
...
- **Corrección**: acierto con refuerzo, fallo con la corrección y la explicación, desarrollo corto con la
  ...
  Toda pregunta enseña su cita, y la que no ancló enseña por qué (en el Examen real no: solo van las
  preguntas, F3-19b).
...
15. Pestaña "Pruebas" con sus cuatro estados; selector de número de preguntas; botones de generar con
    progreso; `AssessmentSolver` en modo práctica con pistas, el material a la vista, corrección al
    entregar y "esto sí lo dije" sobre la corrección.
...
| Modo práctica          | F3-11 a F3-13  | Resolver con las pistas y el material a la vista, y el chat del tutor abierto; sin reloj. Entregar y ver la corrección sin penalización (nota bruta escalada). Devtools: la respuesta de `/solvable` **no contiene** `correctOptionId` ni `expectedAnswer`                                                                                                                                                                                                                                                                                                                       |
...
| Historial de intentos  | F3-39e         | Con una prueba que tenga varios intentos, pulsar "Ver intentos": salen todos por fecha con su modo y su estado. Abrir uno corregido: se ve entero con las respuestas del alumno, las correcciones, la cita de cada pregunta y "esto sí lo dije". Abrir uno cancelado: enseña el motivo y sus interrupciones, sin corrección. Un intento en curso no ofrece abrirse                                                                                                                                                                                                              |
