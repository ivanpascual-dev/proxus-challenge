...
export interface RateLimiter {
  readonly check: (key: string, family: RateLimiterFamily) => Effect.Effect<void, RateLimited>;
...
export const make = (now: () => number = Date.now): Effect.Effect<RateLimiter> => Effect.gen(function* () {
  ...
  const recentHits = (
    ...
    windowMs: number,
    t: number
  ) => (hits.get(bucketKey) ?? []).filter((timestamp) => t - timestamp < windowMs);
  ...
  const checkBucket = (
    ...
    limitName: string,
    t: number
  ): Effect.Effect<readonly number[], RateLimited> => Effect.gen(function* () {
    ...
    if (recent.length >= limit.limit) {
      ...
      return yield* new RateLimited({
        ...
        message: `Se superó el límite de ${limit.limit} peticiones cada ${Math.round(limit.windowMs / 1_000)} s.`
      });
    }
    return recent;
  });

  const check: RateLimiter["check"] = (key, family) => Effect.gen(function* () {
    const t = now();
    const windowBucket = `${family}:window:${key}`;
    const dayBucket = `${family}:day:${key}`;

    const windowRecent = yield* checkBucket(windowBucket, windowLimitFor(family), `${family}PerWindow`, t);
    const dayRecent = yield* checkBucket(dayBucket, dayLimitFor(family), `${family}PerDay`, t);
  ...
  const acquire: RateLimiter["acquire"] = (key) => Effect.gen(function* () {
    ...
    yield* Ref.update(concurrencyRef, (map) => new Map(map).set(key, current + 1));
  });

  const release: RateLimiter["release"] = (key) => Ref.update(concurrencyRef, (map) => {
...
export const layer = (now?: () => number) => Layer.effect(RateLimiter)(make(now));
